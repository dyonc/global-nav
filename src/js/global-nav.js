import { dyonicProducts } from './product-details';
...
function createMobileDropdown(products) {
  ...
  const mobileDropdown = `<li id="all-dyonic-mobile" class="u-hide">
    <ul class="p-navigation__items">
      ...
      <li class="p-navigation__item--dropdown-toggle global-nav__dropdown-toggle">
        <a href="#also-from-dyonic" class="p-navigation__link global-nav__header-link-anchor">Also from Dyonic</a>
        <ul id="also-from-dyonic" class="p-navigation__dropdown">
          ${mobileOthers}
        </ul>
      </li>
      <li class="p-navigation__item--dropdown-toggle global-nav__dropdown-toggle">
      ...
      <div class="global-nav__row is-bordered">
        ...
        <div class="global-nav__flex-container row u-no-padding">
          <div class="global-nav__others-col col-8 col-medium-6">
            <span class="global-nav__muted-heading">Also from Dyonic</span>
...
function showAppropriateNavigation(breakpoint) {
  /* eslint-disable */
  const desktopNav = document.getElementById('all-dyonic');
  const mobileNav = document.getElementById('all-dyonic-mobile');
  const overlay = document.getElementById('all-dyonic-overlay');
...
function addListeners(wrapper, breakpoint) {
  const primaryDropdownCTA = wrapper.querySelector('#all-dyonic-link');
...
export const createNav = ({ breakpoint = 620 } = {}) => {
  ...
  const overlay = createFromHTML(
    '<div id="all-dyonic-overlay" class="global-nav-overlay"></div>'
  );

  const navItem =
    createFromHTML(`<li class="p-navigation__item--dropdown-toggle global-nav__dropdown-toggle u-hide" id="all-dyonic">
      <a href="#dyonic-products" aria-controls="dyonic-products" class="p-navigation__link global-nav__header-link-anchor" id="all-dyonic-link" aria-expanded="false">All Dyonic</a>
    </li>`);

  const mobileDropdownHTML = createMobileDropdown(dyonicProducts);
  const mobileDropdown = createFromHTML(mobileDropdownHTML);

  const navDropdown = createFromHTML(
    `<div class="global-nav__dropdown">
      <div class="global-nav__dropdown-content u-hide" aria-hidden="true" id="dyonic-products">
        ${createProductDropdown(dyonicProducts)}
      </div>
    </div>`
  );
